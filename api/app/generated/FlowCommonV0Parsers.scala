 ...
 * apibuilder:0.12.82 https://app.apibuilder.io/flow/common/0.4.11/anorm_2_x_parsers
 */
import anorm._
...
package io.flow.common.v0.anorm.parsers {
  ...
  object AvailabilityStatus {
    ...
    def parser(name: String = "availability_status"): RowParser[io.flow.common.v0.models.AvailabilityStatus] = {
  ...
  object DayOfWeek {
    ...
    def parser(name: String = "day_of_week"): RowParser[io.flow.common.v0.models.DayOfWeek] = {
      SqlParser.str(name) map {
  ...
  object UserStatus {
    ...
    def parser(name: String = "user_status"): RowParser[io.flow.common.v0.models.UserStatus] = {
      SqlParser.str(name) map {
  ...
  object CatalogItemSummary {
    ...
    def parser(
      ...
      SqlParser.str(name) ~
      SqlParser.get[Map[String, String]](attributes) map {
  ...
  object LineItemForm {
    ...
    def parserWithPrefix(prefix: String, sep: String = "_") = parser(
      number = s"$prefix${sep}number",
      quantity = s"$prefix${sep}quantity",
      shipmentEstimatePrefix = s"$prefix${sep}shipment_estimate",
      pricePrefix = s"$prefix${sep}price",
      attributes = s"$prefix${sep}attributes",
      center = s"$prefix${sep}center",
      discountPrefix = s"$prefix${sep}discount"
    ...
    def parser(
      ...
      SqlParser.str(number) ~
      SqlParser.long(quantity) ~
      io.flow.common.v0.anorm.parsers.DatetimeRange.parserWithPrefix(shipmentEstimatePrefix).? ~
      io.flow.common.v0.anorm.parsers.Money.parserWithPrefix(pricePrefix).? ~
      SqlParser.get[Map[String, String]](attributes).? ~
      SqlParser.str(center).? ~
      io.flow.common.v0.anorm.parsers.Money.parserWithPrefix(discountPrefix).? map {
  ...
  object User {
    ...
    def parser(
      ...
      SqlParser.str(email).? ~
      io.flow.common.v0.anorm.parsers.Name.parserWithPrefix(namePrefix) ~
      io.flow.common.v0.anorm.parsers.UserStatus.parser(status) map {
